...
// `essayResult` (essay-data.js) and the sample roster in
// essay-students-data.js.
//
...
//
// v2 (used by essay-result-v2.html) adds the responsive fixes from
// essay-result-responsive-check.md, a student dropdown, an attempt
// switcher, and a "Show question list" toggle on phones and tablets.
...
// Short pill labels for phones and tablets, where the full ones don't fit
// one row. A status without an entry keeps its full label.
const SHORT_STATUS_LABELS = { "All Questions Marked": "Marked" };

let studentIndex = 0;
let attemptNumber = ESSAY_STUDENTS[0].attempts.length;
let currentResult = null;

// The result on screen: the base attempt from essay-data.js with the
// selected student's and attempt's header details.
function buildResult(student, number) {
  const attempt = student.attempts[number - 1];
  return {
    ...essayResult,
    studentName: student.name,
    attempts: `${number}/${student.attempts.length}`,
    statusLabel: attempt.statusLabel,
    totalMarks: attempt.totalMarks,
    percentage: Math.round((attempt.totalMarks / essayResult.maxMarks) * 100),
  };
}

// Read by essay-pdf-generator-v2.js, so the PDF matches what's on screen.
function currentEssayResult() {
...
// docked column. Keep in sync with the "Tablet & mobile" @media block in
// essay-result-v2.html.
const COMPACT_LAYOUT_QUERY = "(max-width: 1024px)";
const compactLayout = window.matchMedia(COMPACT_LAYOUT_QUERY);

// Question whose feedback the panel shows.
let activeQuestion = null;
let activeView = "summary";
// Question card the reader is on, tracked while scrolling. The view rail
// opens feedback for this question rather than whichever was opened last.
let questionInView = null;
let panelOpen = false;

function setPanelOpen(open) {
  panelOpen = open;
  const slot = document.getElementById("panelSlot");
  // The docked panel shows unless it's --closed; the compact sheet stays
  // hidden unless it's --open, so it can't flash up before this runs.
  slot.classList.toggle("panel-slot--open", open);
  slot.classList.toggle("panel-slot--closed", !open);
  syncRailState();
}

// A view only reads as active while the panel is actually showing it.
function syncRailState() {
  document.querySelectorAll("#panelRail .rnav").forEach((btn) => {
    btn.classList.toggle("rnav--active", panelOpen && btn.dataset.view === activeView);
  });
}

function renderPanel() {
  const q = currentResult.questions.find((x) => x.number === activeQuestion);
  if (!q) return;
...
// Opens the Summary view with the card for one inline highlight expanded
// and scrolled into view. `annoId` is "<question>-<number>", the data-anno
// value a highlight and its card share.
function showAnnotation(annoId) {
  const questionNumber = annoId.split("-")[0];
  setQuestionInView(questionNumber);
  ...
  const scroll = document.getElementById("panelScroll");
  const card = scroll.querySelector(`.fb-card[data-anno="${annoId}"]`);
  if (!card) return;
  card.classList.add("fb-card--open");
  // Scroll the panel directly: scrollIntoView() can also scroll the page
  // behind a fixed-position sheet on some mobile browsers.
  scroll.scrollTop += card.getBoundingClientRect().top - scroll.getBoundingClientRect().top - 12;
}
...
function setQuestionInView(number) {
  questionInView = number;
  document.querySelectorAll("#qrail .qnode").forEach((node) => {
    node.classList.toggle("qnode--active", node.dataset.q === number);
  });
}

// Whichever question card crosses a thin band 40% down the viewport is the
// one being read. The implicit (viewport) root covers both layouts: the
// desktop's inner essay pane and the compact layout's page scroll.
function observeQuestionInView() {
  questionObserver?.disconnect();
  questionObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) setQuestionInView(entry.target.dataset.q);
      });
    },
    { rootMargin: "-40% 0px -55% 0px" }
  );
  document.querySelectorAll("#mainScroll .qblock").forEach((block) => questionObserver.observe(block));
}

// ---------- Question list (phones and tablets) ----------

// On compact layouts the question list stays hidden until the reader asks
// for it; opening it narrows the question cards to make room (CSS).
let questionListOpen = false;

function setQuestionListOpen(open) {
  questionListOpen = open;
  document.querySelector(".stage").classList.toggle("stage--qlist-open", open);
  const toggle = document.getElementById("questionListToggle");
  if (!toggle) return;
  toggle.textContent = open ? "Hide question list" : "Show question list";
  toggle.setAttribute("aria-expanded", String(open));
}

// ---------- Compact-layout sheet ----------

// The sheet covers most of the essay, so it closes the way sheets usually
// do: a tap outside it, or Escape.
function wireSheetDismissal() {
  document.addEventListener("click", (e) => {
    if (!compactLayout.matches || !panelOpen) return;
    // Taps on the sheet, on anything that opens or switches it, or on the
    // download menu leave it open.
    const keepsOpen = e.target.closest(
      "#panelSlot, #panelRail, [data-see-result], .anno, #downloadMenu, #downloadMenuList"
    );
    if (!keepsOpen) setPanelOpen(false);
  });

  document.addEventListener("keydown", (e) => {
    if (e.key !== "Escape" || !compactLayout.matches || !panelOpen) return;
    // An open download menu takes the first Escape
    // (essay-pdf-generator-v2.js closes it).
    if (!document.getElementById("downloadMenuList").hidden) return;
    setPanelOpen(false);
  });
}
...
function renderStudentOptions() {
  document.getElementById("studentSelect").innerHTML = ESSAY_STUDENTS
    .map((student, index) => `<option value="${index}">${escapeHtml(student.name)}</option>`)
    .join("");
}

function renderShell() {
  const result = currentResult;
  const attemptCount = ESSAY_STUDENTS[studentIndex].attempts.length;
  ...
  document.getElementById("studentSelect").value = String(studentIndex);
  document.getElementById("attempts").textContent = result.attempts;
  document.getElementById("attemptPrev").disabled = attemptNumber <= 1;
  document.getElementById("attemptNext").disabled = attemptNumber >= attemptCount;

  // Both labels are rendered; CSS shows the short ones on compact layouts.
  const shortStatus = SHORT_STATUS_LABELS[result.statusLabel] || result.statusLabel;
  document.getElementById("statusPill").innerHTML = `
    <span class="pill__label--full">${escapeHtml(result.statusLabel)}</span>
    <span class="pill__label--short">${escapeHtml(shortStatus)}</span>`;
  document.getElementById("marksPill").innerHTML = `
    ...
    <span class="pill__label--full">Score: ${result.totalMarks}/${result.maxMarks} Marks (${result.percentage}%)</span>
    <span class="pill__label--short">Score: ${result.percentage}%</span>`;
}

function renderRail() {
  const rail = document.getElementById("qrail");
  rail.innerHTML = currentResult.questions
    .map((q, i) => `<button class="qnode${i === 0 ? " qnode--active" : ""}" data-q="${q.number}">${q.number}</button>`)
    .join("");
  rail.querySelectorAll(".qnode").forEach((btn) => {
    btn.addEventListener("click", () => {
      setQuestionInView(btn.dataset.q);
...
function renderQuestions() {
  ...
  // The toggle only matters with more than one question. CSS hides it on
  // desktop, where the question rail is always shown.
  const toggleRow = currentResult.questions.length > 1
    ? `<div class="qlist-toggle-row">
         <button class="qlist-toggle" id="questionListToggle" type="button" aria-controls="qrail" aria-expanded="${questionListOpen}">${questionListOpen ? "Hide" : "Show"} question list</button>
       </div>`
    : "";
  main.innerHTML = toggleRow + currentResult.questions.map(questionCardHtml).join("");

  document.getElementById("questionListToggle")?.addEventListener("click", () => {
  ...
  main.querySelectorAll("[data-see-result]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      setQuestionInView(activeQuestion);
      setPanelOpen(true);
      renderPanel();
    });
  });

  // Highlights open their feedback card. Their `title` tooltip only shows
  // on mouse hover, so this is the way in on touch screens.
  main.querySelectorAll(".anno").forEach((anno) => {
    anno.addEventListener("click", () => showAnnotation(anno.dataset.anno));
    anno.addEventListener("keydown", (e) => {
      if (e.key !== "Enter" && e.key !== " ") return;
      e.preventDefault();
      showAnnotation(anno.dataset.anno);
    });
  });
}

// Renders everything for the selected student and attempt.
function showResult() {
  currentResult = buildResult(ESSAY_STUDENTS[studentIndex], attemptNumber);
  renderShell();
  renderRail();
  renderQuestions();
  if (currentResult.questions.length < 2) setQuestionListOpen(false);

  if (!currentResult.questions.some((q) => q.number === activeQuestion)) {
    activeQuestion = currentResult.questions[0].number;
  }
  setQuestionInView(activeQuestion);
  observeQuestionInView();
  renderPanel();
}

function wireStudentSwitchers() {
  document.getElementById("studentSelect").addEventListener("change", (e) => {
    studentIndex = Number(e.target.value);
    // A newly picked student opens on their latest attempt
    attemptNumber = ESSAY_STUDENTS[studentIndex].attempts.length;
    showResult();
  });

  const prev = document.getElementById("attemptPrev");
  const next = document.getElementById("attemptNext");
  const stepAttempt = (step, button, opposite) => {
    const target = attemptNumber + step;
    if (target < 1 || target > ESSAY_STUDENTS[studentIndex].attempts.length) return;
    attemptNumber = target;
    showResult();
    // Reaching the first or last attempt disables this chevron; keep
    // keyboard focus on the switcher instead of losing it.
    if (button.disabled && !opposite.disabled) opposite.focus();
  };
  prev.addEventListener("click", () => stepAttempt(-1, prev, next));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setPanelOpen(!compactLayout.matches);

  // Crossing the breakpoint (e.g. rotating a tablet) resets the panel to
  // that layout's default instead of leaving a sheet over the essay.
  compactLayout.addEventListener("change", (e) => setPanelOpen(!e.matches));

  document.querySelectorAll("#panelRail .rnav").forEach((btn) => {
    btn.addEventListener("click", () => {
      activeView = btn.dataset.view;
      activeQuestion = questionInView;

// Renders qr-essay.html: the Essay (Composition Writing) result view from
// `essayResult` (essay-data.js), without the student dropdown and attempt
// switcher that essay-render-v2.js adds. Everything else matches v2: the
// responsive fixes from essay-result-responsive-check.md, the "Show
// question list" toggle on phones and tablets, and the feedback sheet.
//
...
function observeQuestionInView() {
  ...
  document.querySelectorAll("#mainScroll .qblock").forEach((block) => observer.observe(block));
}
...
function renderShell() {
  ...
  const shortStatus = SHORT_STATUS_LABELS[essayResult.statusLabel] || essayResult.statusLabel;
  document.getElementById("statusPill").innerHTML = `
    <span class="pill__label--full">${escapeHtml(essayResult.statusLabel)}</span>
  ...
  document.getElementById("marksPill").innerHTML = `
    ...
    <span class="pill__label--full">Score: ${essayResult.totalMarks}/${essayResult.maxMarks} Marks (${essayResult.percentage}%)</span>
    <span class="pill__label--short">Score: ${essayResult.percentage}%</span>`;
}
...
function renderQuestions() {
  ...
  const toggleRow = essayResult.questions.length > 1
    ? `<div class="qlist-toggle-row">
         <button class="qlist-toggle" id="questionListToggle" type="button" aria-controls="qrail" aria-expanded="false">Show question list</button>
       </div>`
    : "";
  main.innerHTML = toggleRow + essayResult.questions.map(questionCardHtml).join("");

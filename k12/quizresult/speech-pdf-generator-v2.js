...
//   2. The transcript prints as text, with every flagged word underlined
//      and given a superscript reference number. Word state is spelled out
//      in the notes below, never carried by colour.
//   3. The Speech Practice Score prints as labelled bars (Accuracy,
//      Pronunciation, Articulation, Fluency, Prosody, each /100).
//   4. Rubrics print fully expanded — no accordion to open.
//
// Three export levels (chosen from the header dropdown):
//   "results"     - transcript, speech scores and rubric scores only
//   "withAnswers" - adds each criterion's band descriptors
//   "full"        - adds pronunciation notes and the AI remarks
//
// Like the other two PDFs, the page is one ink tone throughout: correctness
// is conveyed by underline style, text labels and layout, never by hue.
...
(function () {
  ...
  // Printed label for each word state — the paper stand-in for the live
  // app's green/red word colouring.
  const WORD_STATE_LABEL = {
    "phoneme-mistake": "Mispronounced sound",
    deletion: "Not picked up",
    normal: "Clear",
  };

  // jsPDF's built-in Helvetica is WinAnsi-encoded, so anything outside
  // Latin-1 renders as garbage and throws the line metrics off — which is
  // easy to hit here, because pronunciation feedback invites IPA symbols.
  // Transliterate the ones that are likely to turn up and drop the rest,
  ...
  function buildPdf(data, level) {
    ...
    // Flags are numbered in reading order; the same numbers key the
    // pronunciation notes printed below the transcript.
    function transcriptWords(q, numbered) {
      let n = 0;
      // Sanitised here rather than at draw time because this path measures
      // each word with getTextWidth, which the doc.text wrapper can't cover.
      return q.transcript.map((w) => {
        if (typeof w === "string") return { text: pdfSafe(w), flagged: false, ref: null };
        n += 1;
        return { text: pdfSafe(w.t), flagged: true, ref: numbered ? n : null };
      });
    }

    function drawTranscript(words, opts = {}) {
      const { size = 10, gap = 4, x = contentX, width = contentWidth, lineHeight = 5.8 } = opts;
      ...
      words.forEach((w) => {
        doc.setFont("helvetica", w.flagged ? "bold" : "normal");
        doc.setFontSize(size);
        ...
        if (w.flagged) {
          ...
          doc.rect(cx - 0.4, y - 3.6, wWidth + 0.8, 4.7, "F");
        }
        ...
        if (w.flagged) {
          // Underline doubles the highlight cue, so a flagged word is still
          // obvious if the grey wash washes out on a cheap printer.
          doc.setDrawColor(...COLOR.rule);
          doc.setLineWidth(0.3);
          doc.line(cx, y + 1.2, cx + wWidth, y + 1.2);
        }

        if (w.ref) {
          doc.setFont("helvetica", "bold");
          doc.setFontSize(refSize);
          doc.text(String(w.ref), cx + wWidth + 0.3, y - 1.7);
          doc.setFontSize(size);
        }
    ...
    function drawNoteItem(num, w) {
      const padding = 3;
      const numGutter = 7;
      const textX = contentX + padding + numGutter;
      const textWidth = contentWidth - padding * 2 - numGutter;

      const heading = `"${w.t}"  —  ${WORD_STATE_LABEL[w.state] || w.state}`;
      const headLines = wrapText(heading, textWidth, 9.5, "bold");
      const bodyLines = wrapText(w.note || "", textWidth, 9, "normal");
      const boxHeight = headLines.length * 4.6 + bodyLines.length * 4.3 + padding * 2 + 1.5;
    ...
    function drawPronunciationNotes(q) {
      const flagged = q.transcript.filter((w) => typeof w !== "string");
      if (!showNotes || !flagged.length) return;

      const counts = Object.keys(WORD_STATE_LABEL)
        .map((k) => ({ k, n: flagged.filter((w) => w.state === k).length }))
        .filter((c) => c.n > 0)
        .map((c) => `${WORD_STATE_LABEL[c.k]}: ${c.n}`)
        .join("   ·   ");

      y += 2.5;
      drawSectionLabel(`Pronunciation Notes  (${flagged.length} items)`);
      drawParagraph(counts, { size: 9, gap: 2.5 });
      flagged.forEach((w, i) => drawNoteItem(i + 1, w));
      y += 1.5;
    }
    ...
    function drawQuestion(q) {
      ...
      drawSectionLabel("What we heard:");
      drawTranscript(transcriptWords(q, showNotes), { size: 10, gap: 3 });

      drawPronunciationNotes(q);
      drawSpeechScore(q);
      drawRubrics(q);

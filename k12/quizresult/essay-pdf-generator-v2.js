...
(function () {
  ...
  function download(level) {
    ...
    setTimeout(() => {
      try {
        // The result on screen, from the page's render script
        // (essay-render-v2.js or qr-essay-render.js)
        const result = currentEssayResult();
        const doc = buildPdf(result, level);
        const suffix =
          level === "results" ? "Results" : level === "withAnswers" ? "With-Rubrics" : "Full-Feedback";
        const worksheet = (result.worksheetTitle || result.title).replace(/[^\w]+/g, "-");
        doc.save(`${result.studentName.replace(/\s+/g, "-")}-${worksheet}-${suffix}.pdf`);
      } finally {
        btn.disabled = false;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function openMenu() {
      ...
      // relative to the button's current on-screen location. It's shown
      // first so its width can be measured, then right-aligned to the
      // button but kept at least 12px from the left edge — right-aligning
      // alone pushed it 12px off screen on 360px-wide phones.
      list.hidden = false;
